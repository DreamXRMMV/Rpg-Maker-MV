/*:
 * @plugindesc v1.01 Choose which party members appear as followers or in battle.
 *
...
(function () {
    ...
    var parameters = $plugins.filter(function (p) {
        return p.description.contains('<DreamX Follower and Battle Member Options>');
    })[0].parameters; //Thanks to Iavra

    var parameterMaxIndex = parseInt(parameters['Max Index'] || '12');
    var parameterBattleMembersOnly = String(parameters['Battle Members Only'] || 'true');
    var parameterMaxFollowers = String(parameters['Max Followers'] || '$gameParty.maxBattleMembers() - 1');
    DreamX.FollowerOptions._Game_Interpreter_pluginCommand =
            Game_Interpreter.prototype.pluginCommand;
    Game_Interpreter.prototype.pluginCommand = function (command, args) {
        DreamX.FollowerOptions._Game_Interpreter_pluginCommand.call(this, command, args);
        if (args[0] && parseInt(args[0]) >= 1) {
            var switchNum = DreamX.FollowerOptions.getSwitchNum(args[0]);
            switch (command) {
                case 'ToggleFollower':
                    DreamX.FollowerOptions.ToggleFollower(switchNum);
    ...
    DreamX.FollowerOptions.getSwitchNum = function (actorId) {
        var switchNum = $dataActors[actorId].meta.no_follow_switch ? $dataActors[actorId].meta.no_follow_switch : -1;
        return switchNum;
    };
    ...
    Game_Party.prototype.allFollowers = function () {
        ...
        if (eval(parameterBattleMembersOnly)) {
            ...
            followers = this.allMembers().filter(function (actor) {
                return DreamX.FollowerOptions.isFollowerOkay(actor);
            });
        }
    ...
    DreamX.FollowerOptions.isFollowerOkay = function (actor) {
        ...
        if ($dataActors[actor.actorId()].meta.no_follow === '1')
            return false;
        var followSwitch = $dataActors[actor.actorId()].meta.no_follow_switch || -1;
    ...
    Game_Party.prototype.battleMembers = function () {
        var battleMembers = this.allMembers().filter(function (actor) {
            return actor.isAppeared() && DreamX.FollowerOptions.isActorBattleEnabled(actor);
        });
    ...
    DreamX.FollowerOptions.isActorBattleEnabled = function (actor) {
        if ($dataActors[actor.actorId()].meta.no_battle === '1')
            return false;
        var battleSwitch = $dataActors[actor.actorId()].meta.no_battle_switch || -1;
